 ...
 */
 Sonos.prototype.getQueue = function(callback) {
  debug('Sonos.getQueue(' + ((callback) ? 'callback' : '') + ')');
  ...
  var responseTag = 'u:BrowseResponse';

  var tracks = [];

  return this.request(CONTENT_DIRECTORY_ENDPOINT, action, body, responseTag, function(err, data) {
    if(data!=undefined){
      data.forEach(function(d){
        if(d.NumberReturned[0] != '0'){
          metadata = d.Result[0];
          (new xml2js.Parser()).parseString(metadata, function(err, data) {
            if (err) return callback(err, data);
            data['DIDL-Lite'].item.forEach(function(item){
              tracks.push({
                title: util.isArray(item['dc:title']) ? item['dc:title'][0]: null,
                artist: util.isArray(item['dc:creator']) ? item['dc:creator'][0]: null,
                album: util.isArray(item['upnp:album']) ? item['upnp:album'][0]: null,
